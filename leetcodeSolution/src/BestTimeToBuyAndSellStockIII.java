...
public class BestTimeToBuyAndSellStockIII {
	public int maxProfit(int[] prices) {
		...
		for (int i = 0; i < prices.length; i++) {
			...
			if (tmp_profit > max_profit1)
				max_profit1 = tmp_profit;
		...
		for (int j = prices.length - 1; j >= 0; j--) {
			...
			if (tmp_profit > max_profit2)
				max_profit2 = tmp_profit;
		...
		for (int i = 0; i < prices.length; i++) {
			...
			if (tmp_max_profit > sum_max_profit)
				sum_max_profit = tmp_max_profit;
		}
